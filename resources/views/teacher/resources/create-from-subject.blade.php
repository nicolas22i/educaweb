...
@section('dashboard-content')
    <div class="bg-white p-8 rounded-lg shadow-md max-w-2xl mx-auto">
        ...
        <form action="{{ route('teacher.resources.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                <label class="block font-medium text-gray-700 mb-1">Curso</label>
                <input type="text" value="{{ $subject->course->name }}" disabled
                    class="w-full border border-gray-300 rounded px-4 py-2 bg-gray-100 text-gray-600">
                <input type="hidden" name="course_id" value="{{ $subject->course->id }}">
            </div>

            <div>
                <label class="block font-medium text-gray-700 mb-1">Asignatura</label>
