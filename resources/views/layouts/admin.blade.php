...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <header class="fixed top-0 left-0 right-0 z-30 h-16">
        <div class="flex justify-between items-center px-4 h-full">
            ...
            <a href="{{ route('profile.show') }}" class="flex items-center gap-2 px-3 py-1 rounded-lg hover:bg-gray-700 transition user-profile">
                @if(Auth::check())
                ...
                @endif
                <div class="hidden md:block">
                    <p class="text-sm font-medium text-gray-200 leading-tight">{{ Auth::user()->name ?? 'Invitado' }}</p>
                    <p class="text-xs text-gray-400 leading-tight capitalize">ADMINISTRADOR</p>
                </div>
            </a>
        </div>
    </header>
    ...
    <div class="flex pt-16 h-screen">
        <!-- Sidebar oscuro -->
        <aside id="sidebar" class="w-64 fixed left-0 top-16 h-[calc(100vh-4rem)] z-20 transform transition-transform duration-300 lg:translate-x-0 -translate-x-full lg:translate-x-0">
            <nav class="p-4 text-sm h-full flex flex-col">
                <div class="mb-6 px-2">
                    <p class="text-xs uppercase font-semibold text-gray-400 tracking-wider">Panel de administración</p>
                </div>
                
                <ul class="space-y-1 flex-1">
                    <li>
                        <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                            {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                            <i class="bi bi-speedometer2 text-lg {{ request()->routeIs('admin.dashboard') ? 'text-blue-400' : 'text-gray-400' }}"></i>
                            Dashboard
                        </a>
                    </li>

                    <li>
                        <a href="{{ route('admin.users') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                            {{ request()->routeIs('admin.users*') ? 'active' : '' }}">
                            <i class="bi bi-person-gear text-lg {{ request()->routeIs('admin.users*') ? 'text-blue-400' : 'text-gray-400' }}"></i>
                            Usuarios
                        </a>
                    </li>

                    <li>
                        <a href="{{ route('admin.teachers') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                            {{ request()->routeIs('admin.teachers*') ? 'active' : '' }}">
                            <i class="bi bi-person-badge text-lg {{ request()->routeIs('admin.teachers*') ? 'text-blue-400' : 'text-gray-400' }}"></i>
                            Profesores
                        </a>
                    </li>

                    <li>
                        <a href="{{ route('admin.students') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                            {{ request()->routeIs('admin.students*') ? 'active' : '' }}">
                            <i class="bi bi-people-fill text-lg {{ request()->routeIs('admin.students*') ? 'text-blue-400' : 'text-gray-400' }}"></i>
                            Estudiantes
                        </a>
                    </li>

                    <li>
                        <a href="{{ route('admin.courses') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                            {{ request()->routeIs('admin.courses*') ? 'active' : '' }}">
                            <i class="bi bi-easel2-fill text-lg {{ request()->routeIs('admin.courses*') ? 'text-blue-400' : 'text-gray-400' }}"></i>
                            Cursos
                        </a>
                    </li>

                    <li>
                        <a href="{{ route('admin.subjects') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                            {{ request()->routeIs('admin.subjects*') ? 'active' : '' }}">
                            <i class="bi bi-journals text-lg {{ request()->routeIs('admin.subjects*') ? 'text-blue-400' : 'text-gray-400' }}"></i>
                            Asignaturas
                        </a>
                    </li>
                </ul>
